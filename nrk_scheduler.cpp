...
//EDF implementation block
uint8_t EDF18_implemented(){
		uint8_t min_period_task_ID = 0;
    uint16_t min_period = 65000; 
	  for (int task_ID = 0; task_ID < NRK_MAX_TASKS; task_ID++) {
        ...
        if (nrk_task_TCB[task_ID].task_state == READY && nrk_task_TCB[task_ID].next_period != 0) {
...
void _nrk_scheduler() {
    ...
    for (task_ID = 0; task_ID < NRK_MAX_TASKS; task_ID++) {
        ...
        // Note, this step changes nothing about the task's state -- just its suspend flag changes.
				nrk_cur_task_TCB->suspend_flag = 0;
        ...
        if (nrk_task_TCB[task_ID].task_ID != NRK_IDLE_TASK_ID && nrk_task_TCB[task_ID].task_state != FINISHED) {
            ...
            // Hint: next_wakeup is the number of ticks before the task will be woken up again
					if(nrk_task_TCB[task_ID].next_wakeup > _nrk_prev_timer_val) { //Situation 1A
            ...
            // Hint: next_period is the number of ticks until the next period for a task starts
					if(nrk_task_TCB[task_ID].next_period > _nrk_prev_timer_val) { // Situation 1A
							nrk_task_TCB[task_ID].next_period -=  _nrk_prev_timer_val;	
					} else if(nrk_task_TCB[task_ID].next_period < _nrk_prev_timer_val) { //Situation 1B
							printf("\n Deadline Missed for Task %d",task_ID);
		...
		task_ID = EDF18_implemented();
		//printf("EDF and task ID %d \n",task_ID);
		#else
		task_ID = nrk_get_high_ready_task_ID();
		//printf("RMA and task ID %d \n",task_ID);
    #endif
